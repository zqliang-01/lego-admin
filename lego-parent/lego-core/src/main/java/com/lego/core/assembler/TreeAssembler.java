...
public abstract class TreeAssembler<D extends TreeDTO<D>, E extends TreeEntity<E>> extends BaseAssembler<D, E> {
    ...
    private void matchChildren(Long key, List<D> infos, Map<Long, D> idMap, List<D> result) {
        D info = idMap.get(key);
        if (info != null) {
        	for (D dto : infos) {
