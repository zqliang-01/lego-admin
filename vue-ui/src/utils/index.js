...
/**
    * 导出ElTable表格设置单元格格式和宽度
    * @param {*} name  文件名
    * @param {*} domId
    * @param {*} startRowNum  开始行下标
    * @param {*} endRowNum  结束行下标
    * @param {*} dataFormatters  数据格式数组[{'column':列编号,'dataFormat':数据格式}]
    * @param {*} colWidth  列宽数组[{ wpx: 30 }]
    */
export function exportElTableSetCellWidthDataFormat(name, domId, startRowNum, endRowNum, dataFormatters, colWidth) {
  const fix = document.querySelector('.el-table__fixed')
  let sheet
  if (fix) {
    sheet = XLSX.utils.table_to_sheet(
      document.getElementById(domId).removeChild(fix)
    )
    document.getElementById(domId).appendChild(fix)
  } else {
  ...
  for (let i = startRowNum; i <= endRowNum; i++) {
    for (let j = 0; j < dataFormatters.length; j++) {
      var cell = sheet[dataFormatters[j].column + i]
      XLSX.utils.cell_set_number_format(cell, dataFormatters[j].dataFormat)
    }
  }
  sheet['!cols'] = colWidth
  const wopts = {
  ...
  XLSX.utils.book_append_sheet(wbb, sheet, 'sheet1')
  const wbout = XLSX.write(wbb, wopts)
